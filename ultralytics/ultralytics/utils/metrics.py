...
def bbox_iou(
    ...
    Args:
        ...
        xywh (bool, optional): If True, input boxes are in (x, y, w, h) format. If False, input boxes are in (x1, y1,
            x2, y2) format.
        GIoU (bool, optional): If True, calculate Generclsalized IoU.
        DIoU (bool, optional): If True, calculate Distance IoU.
        CIoU (bool, optional): If True, calculate Complete IoU.
        MPDIoU (bool, optional): If True, calculate Minimum Point Distance IoU.
        imgsz (torch.Tensor, optional): Image height and width used for MPDIoU normalization.
        eps (float, optional): A small value to avoid division by zero.
    ...
    if CIoU or DIoU or GIoU or MPDIoU:
        ...
        if MPDIoU:
            if imgsz is None:
                raise ValueError("imgsz must be provided when MPDIoU=True")

            img_h = imgsz[0].to(iou.dtype)
            img_w = imgsz[1].to(iou.dtype)
